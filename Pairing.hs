...
import Control.Monad.Trans.Free hiding (Free, Pure, Free)
...
pairEffect :: (Pairing f g, Monad m)
              => (a -> b -> r) -> Cofree f a -> FreeT g m b -> m r
pairEffect p s c = do
  mb <- runFreeT c
  case mb of
    T.Pure x -> return $ p (extract s) x
    T.Free gs -> pair (pairEffect p) (unwrap s) gs
